 ...
 */
public class KMST_2 extends AbstractKMST {
	...
	public KMST_2(Integer numNodes, Integer numEdges, HashSet<Edge> edges, int k) {
	...
	private void baum(BitSet knoten, int gewicht, int tiefe, Set<Edge> se, int kn, ArrayList<Edge> kanten) {
		...
		this.anz++;
		if (!se.isEmpty() && this.n.contains(se)) return;
		this.n.add(se);
		if (tiefe >= this.k) {
			if (this.max > gewicht) {
				this.max = gewicht;
		...
		PriorityQueue<Edge> kan = new PriorityQueue<Edge>(kanten);
		kan.addAll(this.adjList[kn]);
		kanten.clear();
		kanten.addAll(kan);
		kan = null;
	...
	public void run() {
		...
		for (int i = 0; i < this.v; i++) {
			...
			al.set(i);
			this.baum(al, 0, 1, new HashSet<Edge>(), i, new ArrayList<Edge>());
		}
